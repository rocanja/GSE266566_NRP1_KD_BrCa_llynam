...
subtitle: "GSE266566.NRP1.expression.and.gene.locus"
---
...
dim(fpkm) #78894    52
identical(rownames(fpkm), y$genes$Ensembl.114.Gene.ID) #sanity check
#
rownames(fpkm)<-paste(y$genes$Ensembl.114.Gene.Name,rownames(fpkm),sep=' | ')
#
nrp1.fpkm<-fpkm[grep('^NRP1',rownames(fpkm)),,drop=F]
dim(nrp1.fpkm)
...
AA<-ggplot(nrp1.fpkm, aes(x=Treat, y=`NRP1 | ENSG00000099250.18`))+
	...
	geom_point(aes(color=Treat), position=position_jitter(seed=1, width=0.25, height=0.0), size=2.5, alpha=0.7)+ 
...
```{r}
rm(list=setdiff(ls(),c("AA","nrp1.label","mygtf","level.order.treat","level.order.line","group.colors.treat","group.colors.line"))) #cleanup
```
...
dim(isoPcts) #387954     52
identical(rownames(isoPcts), y$genes$Ensembl.114.Transcript.ID) #sanity check
#
rownames(isoPcts)<-paste(y$genes$Transcript.Name,rownames(isoPcts),y$genes$Transcript.Type,sep=' | ')
#
nrp1.isopct<-isoPcts[grep('^NRP1',rownames(isoPcts)),,drop=F]
dim(nrp1.isopct) #14
...
```{r}
rm(list=setdiff(ls(),c("AA","nrp1.label","nrp1.isopct","B","named.p36","mygtf","level.order.treat","level.order.line","group.colors.treat","group.colors.line"))) #cleanup
gc()
...
library(rtracklayer)
#
### load gtf file with rtracklayer first
mygtf<-rtracklayer::import('ftp://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_48/gencode.v48.primary_assembly.annotation.gtf.gz') #get from Gencode.v48 = Ensembl.v114
class(mygtf) #"GRanges"
...
C<-trancriptVis(gtfFile = mygtf, gene = "NRP1", #now it works
             ...
             #
             annotate("text", x=33185638+2000, y=16.0, label="siNRP1 #3 (5’-UAACCACAUUUCACAAGAA-3’) ... includes NRP1-209", color=group.colors.treat[2], size=3.5, hjust=0) +
             annotate("text", x=33263779+2000, y=16.0, label="siNRP1 #5 (5’-CAGCCUUGAAUGCACUUAU-3’) ... excludes NRP1-213, includes NRP1-207", color=group.colors.treat[3], size=3.5, hjust=0) +
             annotate("text", x=33185659+2000, y=15.4, label="shNRP1 #3 (5ʹ-GCUGUGGAUGACAUUAGUAUU-3ʹ) ... excludes NRP1-209", color=group.colors.treat[5], size=3.5, hjust=0) +
             annotate("text", x=33263779+2000, y=15.4, label="shNRP1 #5 (5’-CAGCCUUGAAUGCACUUAU-3’) ... excludes NRP1-213, includes NRP1-207", color=group.colors.treat[6], size=3.5, hjust=0) +
             #
...
### NRP1 gene locus: chr10:33177492-33336262 (-)
###
...
#
### shNRP1#3, 5ʹ GCUGUGGAUGACAUUAGUAUU 3'   chr10 start 33,185,659 (-) ... misses NRP1-209 (..3802)  
...
#
#
plot<-plot_grid(plot_grid(AA,B,rel_widths=c(1,1.6),labels=c('a','b')), 
...
#  [1] rtracklayer_1.66.0   GenomicRanges_1.58.0 GenomeInfoDb_1.42.3  IRanges_2.40.1      
#  [5] S4Vectors_0.44.0     BiocGenerics_0.52.0  transPlotR_0.0.2     lubridate_1.9.4     
#  [9] forcats_1.0.1        stringr_1.5.2        purrr_1.1.0          readr_2.1.5         
# [13] tidyr_1.3.1          tibble_3.3.0         tidyverse_2.0.0      edgeR_4.4.2         
# [17] limma_3.62.2         cowplot_1.2.0        ggrepel_0.9.6        reshape2_1.4.4      
# [21] dplyr_1.1.4          ggplot2_4.0.0       
# 
# loaded via a namespace (and not attached):
#  [1] tidyselect_1.2.1            farver_2.1.2                Biostrings_2.74.1          
#  [4] S7_0.2.0                    bitops_1.0-9                fastmap_1.2.0              
#  [7] RCurl_1.98-1.17             GenomicAlignments_1.42.0    XML_3.99-0.19              
# [10] digest_0.6.37               timechange_0.3.0            lifecycle_1.0.4            
# [13] statmod_1.5.0               magrittr_2.0.4              compiler_4.4.3             
# [16] rlang_1.1.6                 tools_4.4.3                 yaml_2.3.10                
# [19] knitr_1.50                  S4Arrays_1.6.0              curl_7.0.0                 
# [22] DelayedArray_0.32.0         plyr_1.8.9                  RColorBrewer_1.1-3         
# [25] abind_1.4-8                 rsconnect_1.5.1             BiocParallel_1.40.0        
# [28] withr_3.0.2                 grid_4.4.3                  colorspace_2.1-1           
# [31] scales_1.4.0                SummarizedExperiment_1.36.0 cli_3.6.5                  
# [34] rmarkdown_2.29              crayon_1.5.3                generics_0.1.4             
# [37] rstudioapi_0.17.1           httr_1.4.7                  tzdb_0.5.0                 
# [40] rjson_0.2.23                zlibbioc_1.52.0             parallel_4.4.3             
# [43] XVector_0.46.0              restfulr_0.0.16             matrixStats_1.5.0          
# [46] vctrs_0.6.5                 Matrix_1.7-4                jsonlite_2.0.0             
# [49] hms_1.1.3                   locfit_1.5-9.12             glue_1.8.0                 
# [52] codetools_0.2-20            stringi_1.8.7               gtable_0.3.6               
# [55] BiocIO_1.12.0               UCSC.utils_1.2.0            pillar_1.11.0              
# [58] htmltools_0.5.8.1           GenomeInfoDbData_1.2.13     R6_2.6.1                   
# [61] evaluate_1.0.5              lattice_0.22-7              Biobase_2.66.0             
# [64] Rsamtools_2.22.0            Rcpp_1.1.0                  SparseArray_1.6.2          
# [67] xfun_0.53                   MatrixGenerics_1.18.1       pkgconfig_2.0.3  
##############################
